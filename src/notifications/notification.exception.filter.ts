...
export class ExceptionFilter implements ExceptionFilter {
  ...
  private handleHttp(
    ...
    return of(response.json({ status, message })).pipe(mapTo(undefined));
  }

  private handlePgNotify(
    error: Error & { response?: any },
    context: PgNotifyContext,
  ): Observable<PgNotifyResponse | undefined> {
    const parsedError = this.parseError(error);
    ...
    if (requestId) {
      return of(
        PgNotifyResponse.error(parsedError.message, parsedError.status),
      );
    }
  ...
  private parseError(error: Error & { response?: any }): {
    status: number;
    message: unknown;
  } {
    let status = 500;
